...
- gdp:    GDP per capita, World Bank NY.GDP.PCAP.CD.
- pop:    total population, World Bank SP.POP.TOTL — scatter view bubble size.

Also writes data/region.js (iso3 -> World Bank region, 7 regions) — scatter
view bubble colour.
...
def wb_countries():
    """iso2 -> iso3, normalised-name -> iso3, and iso3 -> region, for real countries."""
    print("downloading World Bank country list ...")
    payload = json.loads(fetch(WB_COUNTRIES_URL))
    iso2to3, name_to3, region = {}, {}, {}
    for row in payload[1]:
        ...
        region[iso3] = row["region"]["value"].strip()
    return iso2to3, name_to3, region
...
def build_pop():
    print(f"downloading World Bank {POP_INDICATOR} ...")
    payload = json.loads(fetch(WB_URL.format(ind=POP_INDICATOR)))
    by_iso = {}
    for row in payload[1]:
        iso = WB_ISO_FIX.get(row["countryiso3code"], row["countryiso3code"])
        if (len(iso) != 3 and iso != "OWID_KOS") or row["value"] is None:
...
def main():
    iso2to3, name_to3, region = wb_countries()
    ...
    js = ("// Generated by scripts/build_extras.py — do not edit by hand.\n"
          ...
          "// pop:    total population (World Bank SP.POP.TOTL) — scatter bubble size\n"
    ...
    region_js = ("// Generated by scripts/build_extras.py — do not edit by hand.\n"
          "// iso3 -> World Bank region (7 regions) — scatter bubble colour\n"
          "window.REGION=" + json.dumps(region, separators=(",", ":")) + ";\n")
    (REPO / "data" / "region.js").write_text(region_js)
    print(f"wrote data/region.js — {len(region)} countries, "
          f"{len(set(region.values()))} regions")
