...
- **Timelapse with history** — press play to watch the real share of people online, country by country, year by year from 1990 to 2024, while the events that shaped the internet (Mosaic, Google, the iPhone, the Arab Spring, Jio, COVID…) appear in sync. Every year has at least one milestone; country-specific ones pulse on the map. Each country's drill-down panel lists every milestone tagged to it — over 75 countries now have at least one.
...
- **Keyboard** — `←`/`→` step the year, `space` plays/pauses, `1`–`0` pick a data layer, `F`/`G`/`S`/`R` switch to flat map / globe / scatter / race, `⌘K`/`Ctrl K` opens a country search palette (type to filter, arrows + enter to jump straight to a country's panel), `Esc` closes the country panel or the search palette.
