...
- **Flat map / 3D globe / Scatter toggle** — view country data on a Natural Earth projection, a draggable orthographic globe, or a Gapminder-style scatter plot with any two of the twelve metrics (including GDP per capita) on the axes — animated through the same 1990–2024 timelapse, log-scaled where the data is long-tailed, click a dot for the full country drill-down.
...
- Internet exchange points and data-center facility counts come from the [PeeringDB](https://www.peeringdb.com/) API (1,311 exchanges and 5,857 facilities, live); native IPv6 adoption is Google's [per-country snapshot](https://www.google.com/intl/en/ipv6/statistics.html) (no history — refreshed daily upstream, pulled at build time). GDP per capita (current US$, full 1990+ history, used as the scatter view's flagship axis) is World Bank indicator `NY.GDP.PCAP.CD`. All three refreshed with `python3 scripts/build_extras.py`.
